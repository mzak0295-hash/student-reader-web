...
def ocr_page(page):
    """OCR عربي للصفحة باستخدام Tesseract مباشرة، بدون pytesseract/Pillow."""
    if fitz is None:
        return ""

    image_path = None
    try:
        # دقة أعلى قليلًا لتحسين قراءة العربية في الصفحات المصورة.
        pix = page.get_pixmap(
        ...
        best = ""
        # أكثر من وضع قراءة؛ نأخذ أطول نتيجة نظيفة.
        for psm in ("6", "3", "4"):
            try:
        ...
        # PyMuPDF OCR fallback إذا كان Tesseract المباشر غير متاح.
        if len(best) < 8:
            try:
                ...
                candidate = clean_extracted_text(
...
def convert_pdf(uploaded_file, progress=None):
    ...
    for page_number, text in enumerate(texts, start=1):
        ...
        if not text:
            # لا نتخطى أي صفحة: نحاول تحويلها برسالة واضحة بدل إسقاطها.
            text = (
                f"الصفحة {page_number}. "
                "تعذر استخراج النص المقروء من هذه الصفحة آليًا."
            )
