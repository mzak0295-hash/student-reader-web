...
async def make_audio(text):
    """إنشاء MP3 فعليًا عبر ملف مؤقت ثم قراءته إلى الذاكرة.

    بعض إصدارات edge-tts لا تقبل BytesIO مع save()؛ لذلك نستخدم
    اسم ملف حقيقيًا، ثم نقرأ الناتج ونحذف الملف المؤقت.
    """
    temp_path = None
    try:
...
def extract_page_texts(uploaded_file):
    """استخراج النص من كل الصفحات بطريقتين، مع تفضيل النص غير الفارغ.
    PyMuPDF غالبًا يتعامل مع ملفات PDF العربية أفضل من pypdf في بعض الملفات.
    """
    raw = uploaded_file.getvalue()
    texts = []

    # الطريقة الأولى: PyMuPDF
    if fitz is not None:
        try:
            doc = fitz.open(stream=raw, filetype="pdf")
            texts = [page.get_text("text", sort=True) or "" for page in doc]
            doc.close()
            if texts and any(t.strip() for t in texts):
                # سنحاول pypdf فقط للصفحات التي بقيت فارغة.
                pass
        except Exception:
            texts = []

    # الطريقة الثانية: pypdf، خصوصًا للصفحات التي لم ينجح معها PyMuPDF.
    try:
        reader = PdfReader(io.BytesIO(raw))
        pypdf_texts = []
        for page in reader.pages:
            try:
                pypdf_texts.append(page.extract_text() or "")
            except Exception:
                pypdf_texts.append("")
    except Exception:
        pypdf_texts = []

    total = max(len(texts), len(pypdf_texts))
    merged = []
    for i in range(total):
        a = texts[i] if i < len(texts) else ""
        b = pypdf_texts[i] if i < len(pypdf_texts) else ""
        # اختر النص الذي يحتوي على حروف أكثر بعد التنظيف.
        ca = clean_extracted_text(a)
        cb = clean_extracted_text(b)
        merged.append(ca if len(ca) >= len(cb) else cb)
    return merged


def convert_pdf(uploaded_file, progress=None):
    texts = extract_page_texts(uploaded_file)
    total = len(texts)
    audio_files = []
    skipped = 0
    ...
    for page_number, text in enumerate(texts, start=1):
        ...
        # إعادة المحاولة مرتين عند أي خطأ مؤقت في خدمة الصوت.
        data = None
        last_error = None
